...
const EventEmitter = require$$0__default.EventEmitter;

const childProcess = require$$1__default;

const path$1 = require$$2__default;

const fs$1 = require$$3__default;
...
class Command extends EventEmitter {
  ...
  constructor(name) {
    ...
    this._showHelpAfterError = false; // see .configureOutput() for docs
...
var choices = resources.map(function (item) {
  return item.name;
}); // 定义模版的问题
...
var initAction = /*#__PURE__*/function () {
  var _ref = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee(dirName, options) {
    var answers, confirm, template, branch, commandStr;
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 17:
            // 3. 下载模版
            template = resources.find(function (item) {
              return item.name == answers.type;
            });

            if (template) {
              branch = options.branch || template.branch;
              commandStr = "git clone ".concat(template.git, " -b ").concat(branch, " ").concat(dirName);
              shell.exec(commandStr);
            } else {
              console.log(chalk.red("".concat(answers.type, "\u6A21\u7248\u4E0D\u5B58\u5728")));
              shell.exit(1);
            } // 清理文件


            rm(path.join(process.cwd(), dirName, '/.git/'), function (err) {
              if (err) throw err;
            });
            _context.next = 25;
            break;
          ...
          case 22:
            ...
            console.log(chalk.red('error', _context.t0));
  ...
  return function initAction(_x, _x2) {
...
var Cli = /*#__PURE__*/function () {
  function Cli(appPath) {
    ...
    this.program = new commander.exports.Command();
    this.init();
  }
  ...
  _createClass(Cli, [{
    key: "init",
    value: function init() {
      this.program.command('init [dirPath]').description('create a project with template project').option('-t, --type <type>', 'template project type, only accept "custom-component", "miniprogram", "plugin", "game"').option('-f, --force', 'all files will be overrided whether it already exists or not').option('-p, --proxy <url>', 'http/https request proxy').option('-n, --newest', 'use newest template to initialize project').action(initAction);
    }
  }, {
    key: "run",
    value: function run() {
